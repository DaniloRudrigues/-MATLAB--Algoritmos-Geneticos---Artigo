function mutationChildren = mut_gaussiana_ind_2(parents,options,GenomeLength,FitnessFcn,state,thisScore,thisPopulation,mutationRate)
...
if(strcmpi(options.PopulationType,'doubleVector'))
    ...
    for i=1:length(parents)
        ...
        for oo = 1:length(lb)
           if  mutationChildren(i,oo) < lb(1,oo)
               mutationChildren(i,oo) = lb(1,oo);
           end
           if mutationChildren(i,oo) > ub(1,oo)
              mutationChildren(i,oo) = ub(1,oo);
           end
        end
    end
